			...
			<div class="container cont-main-frame">
				...
				<div class="main-box">
					...
					<div class="header-title">
						<h1 class="main-title">Hasil Keputusan oleh <p style="text-transform: uppercase; display:inline;">smart</p>
						</h1>
						<hr class="title-line">
					</div>
					<!-- akhir header title -->
						...
						<div class="card-header card-header-custom" id="headingTwo">
							<h2 class="mb-0">
								<button class="btn btn-link btn-block text-left collapsed" type="button" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
									Hasil <strong>Nilai Utility</strong>
								</button>
							</h2>
						</div>
									...
									<thead class="thead-blue">
										<tr>
											...
											<?php foreach ($criteria->result_array() as $ctr) : ?>
												<th class="thead-custom"><?= $ctr['kode']; ?></th>
											<?php endforeach; ?>
										</tr>
									</thead>
									<tbody class="tbody-custom">
										<?php if ($ara->num_rows() > 0) : ?>
											<?php //var_dump($hasil['nilai utility']); die; 
											?>
											<?php //$count = 1; 
											?>
											<?php foreach ($ara->result_array() as $ar) : ?>
												<tr class="tr-striped">
													<th class="border-custom" scope="row"><?= $ar['kode']; ?></th>
													<td class="border-custom"><?= $ar['nama']; ?></td>

													<?php foreach ($criteria->result_array() as $c) : ?>
														<td class="border-custom"><?= $hasil['nilai utility'][$ar['kode']]['nilai' . $c['kode']]; ?></td>
													<?php endforeach; ?>
						...
						<div class="card-header card-header-custom" id="headingThree">
							<h2 class="mb-0">
								<button class="btn btn-link btn-block text-left collapsed" type="button" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
									Hasil <strong>Nilai Akhir</strong>
								</button>
							</h2>
						</div>
						<div id="collapseThree" class="collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
							<div class="card-body">
									...
									<tbody class="tbody-custom">
										...
										<?php foreach ($ara->result_array() as $a) : ?>
											...
											<?php foreach ($ara->result_array() as $alter) : ?>
												...
												<?php if ($alter['kode'] === $hasil['nilai akhir'][$count]['kode']) : ?>
													<?php //var_dump($hasil['vektor V']); die; 
													?>
													<tr class="tr-striped">
														<th class="border-custom" scope="row"><?= $hasil['nilai akhir'][$count]['kode']; ?></th>
														<td class="border-custom"><?= $alter['nama']; ?></td>
														<td class="border-custom"><?= $hasil['nilai akhir'][$count]['nilai']; ?></td>
								...
								<div class="card card-kesimpulan">
									...
									<div class="card-body">
										<p class="card-text">Berdasarkan hasil perhitungan metode Simple Multi-Attribute Rating Technique (SMART) maka alternatif tanaman ara varietas <strong><?= $keputusan['nama'] . " (" . $keputusan['kode'] . ")" ?></strong> merupakan alternatif tanaman ara yang tepat untuk ditanam dengan nilai akhir <?= $hasil['nilai akhir'][0]['nilai'] ?>. </p>
									</div>
								</div>
